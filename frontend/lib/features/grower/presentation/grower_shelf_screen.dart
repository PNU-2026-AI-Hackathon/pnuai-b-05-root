...
/// [size]개보다 적어도(예: 5개 → 3+2) 그대로 둔다 — 억지로 3개씩 채우지 않는다.
...
/// "홈" 탭: 담당 묘목을 "N단 선반"(최대 3개씩) 단위로 세로로 쌓아 보여주는 선반 뷰.
...
class _GrowerShelfScreenState extends RevalidatableState<GrowerShelfScreen> {
  ...
  Widget _buildBody() {
    ...
    final tiers = _chunkIntoTiers(_entries, 3);
...
/// 3) 하향 앰비언트 글로우, 4) 화분 최대 3개(Row), 5) 선반 받침대. 이 5단계를 감싸는
...
/// 4) 화분 최대 3개를 가로로 나란히 배치. 참고 목업의 "flex-1 max-w-[100px]"을
/// `Expanded` + `Center` + `ConstrainedBox(maxWidth: 100)`로 옮겼다 — 화분이 3개보다
/// 적은 마지막 단도 억지로 채우지 않고 있는 개수만큼만 중앙에 고르게 배치된다.
...
/// 패딩/폰트가 이 100px 폭 카드엔 너무 커서 재사용하지 않았다).
